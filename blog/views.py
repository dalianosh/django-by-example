...
def post_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            search_vec = SearchVector('title', 'body')
            search_que = SearchQuery(query)
            results = Post.published.annotate(search=search_vec,
                                              rank=SearchRank(search_vec, search_que)
                                             ).filter(search=search_que).order_by('-rank')
